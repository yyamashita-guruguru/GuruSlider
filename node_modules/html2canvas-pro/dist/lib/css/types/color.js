...
const a98_1 = require("./color-spaces/a98");
const pro_photo_1 = require("./color-spaces/pro-photo");
const rec2020_1 = require("./color-spaces/rec2020");
exports.color = {
    name: 'color',
    parse: (context, value) => {
...
const rgb = (_context, args) => {
    ...
    if (tokens.length === 3) {
        const [r, g, b] = tokens.map(color_utilities_1.getTokenColorValue);
        return (0, color_utilities_1.pack)(r, g, b, 1);
    }
    if (tokens.length === 4) {
        const [r, g, b, a] = tokens.map(color_utilities_1.getTokenColorValue);
        return (0, color_utilities_1.pack)(r, g, b, a);
    }
    ...
    if (tokens.length === 5 && tokens[3].type === 6 /* TokenType.DELIM_TOKEN */ && tokens[3].value === '/') {
        const r = (0, color_utilities_1.getTokenColorValue)(tokens[0], 0);
        const g = (0, color_utilities_1.getTokenColorValue)(tokens[1], 1);
        const b = (0, color_utilities_1.getTokenColorValue)(tokens[2], 2);
        const a = (0, color_utilities_1.getTokenColorValue)(tokens[4], 3);
        return (0, color_utilities_1.pack)(r, g, b, a);
    }
    return 0;
};
...
const _color = (context, args) => {
    const tokens = args.filter(parser_1.nonFunctionArgSeparator), token_1_value = tokens[0].type === 20 /* TokenType.IDENT_TOKEN */ ? tokens[0].value : 'unknown', is_absolute = !(0, color_utilities_1.isRelativeTransform)(tokens);
    if (is_absolute) {
        ...
        const c1 = (0, parser_1.isNumberToken)(tokens[1]) ? tokens[1].number : 0, c2 = (0, parser_1.isNumberToken)(tokens[2]) ? tokens[2].number : 0, c3 = (0, parser_1.isNumberToken)(tokens[3]) ? tokens[3].number : 0, a = tokens.length > 4 &&
        ...
        const extractComponent = (color, token) => {
            ...
            const parseCalc = (args) => {
                const parts = args.filter(parser_1.nonFunctionArgSeparator);
                let expression = '(';
            ...
            if (token.type === 18 /* TokenType.FUNCTION */) {
                const args = token.values.filter(parser_1.nonFunctionArgSeparator);
                if (token.name === 'calc') {
        ...
        let from = tokens[1].type === 18 /* TokenType.FUNCTION */ ? tokens[1].values : (0, parser_1.isIdentToken)(tokens[1]) ? [tokens[1]] : [];
        if ((0, parser_1.isIdentToken)(tokens[1])) {
            const named_color = exports.COLORS[tokens[1].value.toUpperCase()];
            if (typeof named_color === 'undefined') {
                throw new Error(`Attempting to use unknown color in relative color 'from'`);
            }
            else {
                const _c = (0, exports.parseColor)(context, tokens[1].value), alpha = 0xff & _c, blue = 0xff & (_c >> 8), green = 0xff & (_c >> 16), red = 0xff & (_c >> 24);
                from = [
