...
var PerformanceMonitor = class {
  ...
  */
  start(name, metadata) {
    ...
    if (this.activeMetrics.has(name)) (_a8 = this.context) == null ? void 0 : _a8.logger.warn(`Performance metric '${name}' already started. Overwriting.`);
  ...
  */
    ...
    if (!metric) {
      (_a8 = this.context) == null ? void 0 : _a8.logger.warn(`Performance metric '${name}' not found. Was start() called?`);
      return;
    }
...
var cssFormattedClasses = function(codePoints, options) {
  ...
  var _a8 = codePointsToCharacterClasses(codePoints, options.lineBreak), indicies = _a8[0], classTypes = _a8[1], isLetterNumber = _a8[2];
...
var LineBreaker = function(str, options) {
  ...
  var _a8 = cssFormattedClasses(codePoints, options), indicies = _a8[0], classTypes = _a8[1], forbiddenBreakpoints = _a8[2];
  var length = codePoints.length;
  var lastEnd = 0;
  var nextIndex = 0;
...
var safeEvalArithmetic = (expr) => {
  ...
  for (const item of output) if (typeof item === "number") stack.push(item);
    ...
    const b = stack.pop();
    const a2 = stack.pop();
    if (a2 === void 0 || b === void 0) return NaN;
    switch (item) {
      case "+":
        stack.push(a2 + b);
        break;
...
var packLab = (_context, args) => {
  ...
  const [l, a2, b, alpha] = extractLabComponents(args), rgb2 = srgbLinear2rgb(xyz2rgbLinear(lab2xyz([
    l,
    a2,
    b
  ])));
...
var packOkLab = (_context, args) => {
  ...
  const [l, a2, b, alpha] = extractLabComponents(args), rgb2 = srgbLinear2rgb(xyz2rgbLinear(oklab2xyz([
    l,
    a2,
    b
  ])));
...
var packLch = (_context, args) => {
  ...
  const [l, c, h, a2] = extractLchComponents(args), rgb2 = srgbLinear2rgb(xyz2rgbLinear(lab2xyz(lch2lab([
    l,
    c,
    h
  ]))));
...
var packHSL = (context, args) => {
  ...
  const [h, s, l, a2] = extractHslComponents(context, args), rgb2 = hsl2rgb([
    h,
    s,
    l
  ]);
...
var lab2xyz = (lab) => {
  const fy = (lab[0] + 16) / 116, fx = lab[1] / 500 + fy, fz = fy - lab[2] / 200, k = 24389 / 27, e2 = 24 / 116, xyz = [
    ...
    (fz > e2 ? fz ** 3 : (116 * fz - 16) / k) * 0.2958 / 0.3585
  ];
  return d50toD65([
    xyz[0],
    xyz[1],
...
var rgbToXyz = (_context, args) => {
  ...
  if (tokens.length === 4) {
    const [r, g, b, a2] = tokens.map(getTokenColorValue), rgb_linear = rgb2rgbLinear([
      r / 255,
      g / 255,
      b / 255
    ]), [x, y, z2] = rgbLinear2xyz([
...
var hslToXyz = (context, args) => {
  const [h, s, l, a2] = extractHslComponents(context, args), rgb_linear = rgb2rgbLinear(hsl2rgb([
    h,
    s,
    l
  ])), [x, y, z2] = rgbLinear2xyz([
...
var labToXyz = (_context, args) => {
  const [l, a2, b, alpha] = extractLabComponents(args), [x, y, z2] = lab2xyz([
    l,
    a2,
    b
  ]);
...
var lchToXyz = (_context, args) => {
  const [l, c, h, alpha] = extractLchComponents(args), [x, y, z2] = lab2xyz(lch2lab([
    l,
    c,
    h
  ]));
...
var oklchToXyz = (_context, args) => {
  const [l, c, h, alpha] = extractOkLchComponents(args), [x, y, z2] = oklab2xyz(lch2lab([
    l,
    c,
    h
  ]));
...
var oklabToXyz = (_context, args) => {
  const [l, c, h, alpha] = extractLabComponents(args), [x, y, z2] = oklab2xyz([
    l,
    c,
    h
  ]);
...
var _color = (context, args) => {
  ...
  if (!isRelativeTransform(tokens)) {
    ...
    const from_final_colorspace = toColorFromXyz(fromColorToXyz(context, from)), c1 = extractComponent(from_final_colorspace, tokens[3]), c2 = extractComponent(from_final_colorspace, tokens[4]), c3 = extractComponent(from_final_colorspace, tokens[5]), a2 = tokens.length > 6 && tokens[6].type === 6 && tokens[6].value === "/" && isNumberToken(tokens[7]) ? tokens[7].number : 1;
...
var DocumentCloner = class {
  ...
  toIFrame(ownerDocument, windowSize) {
    ...
    try {
      const ownerWindow = (_a8 = this.referenceElement.ownerDocument) == null ? void 0 : _a8.defaultView;
  ...
  createStyleClone(node) {
    try {
      ...
      if (e2.name !== "SecurityError") throw e2;
    }
  ...
  createCanvasClone(canvas) {
    ...
    try {
      ...
      if (clonedCtx) if (!this.options.allowTaint && ctx) clonedCtx.putImageData(ctx.getImageData(0, 0, canvas.width, canvas.height), 0, 0);
        ...
        if (gl) {
          if (((_a8 = gl.getContextAttributes()) == null ? void 0 : _a8.preserveDrawingBuffer) === false) this.context.logger.warn("Unable to clone WebGL context as it has preserveDrawingBuffer=false", canvas);
        }
...
var equalPath = (a2, b) => {
  if (a2.length === b.length) return a2.some((v, i) => v === b[i]);
  return false;
};
...
var ElementPaint = class {
  constructor(container, parent) {
    ...
    if (this.container.styles.zoom !== 1) {
      ...
      const offsetX = this.container.bounds.left + getAbsoluteValue(origin[0], this.container.bounds.width);
      const offsetY = this.container.bounds.top + getAbsoluteValue(origin[1], this.container.bounds.height);
      const z2 = this.container.styles.zoom;
      const zoomMatrix = [
        z2,
        0,
        0,
...
var FontMetrics = class {
  ...
  getMetrics(fontFamily2, fontSize2) {
    ...
    if (typeof this._data[key] === "undefined") this._data[key] = this.parseMetrics(fontFamily2, fontSize2);
...
var getTextStrokeLineJoin = () => {
  var _a8;
  return ((_a8 = typeof window !== "undefined" ? window : void 0) == null ? void 0 : _a8.chrome) ? "miter" : "round";
};
var TextRenderer = class {
  ...
  */
  renderLineClampedText(text, styles, paintOrder2, containerBounds) {
    ...
    for (let i = 0; i < maxLines - 1; i++) lines[i].forEach((tb) => this.renderTextBoundWithPaintOrder(tb, styles, paintOrder2));
    const lastLine = lines[maxLines - 1];
    if ((lastLine == null ? void 0 : lastLine.length) && containerBounds) {
  ...
  */
  renderEllipsisText(text, styles, paintOrder2, containerBounds) {
    const lineHeight2 = styles.fontSize.number * 1.5;
    ...
    for (const layer of paintOrder2) if (layer === 0) this.renderTextFillWithShadows(bounds, styles);
    else if (layer === 1) this.renderTextStrokeWithStyle(bounds, styles);
    return true;
  }
  async renderTextNode(text, styles, containerBounds) {
    ...
    if (styles.webkitLineClamp > 0 && (styles.display & 2) !== 0 && styles.overflowY === 1 && text.textBounds.length > 0) {
      if (this.renderLineClampedText(text, styles, paintOrder2, containerBounds)) return;
    }
    if (styles.textOverflow === 1 && containerBounds && styles.overflowX === 1 && text.textBounds.length > 0 && this.renderEllipsisText(text, styles, paintOrder2, containerBounds)) return;
    text.textBounds.forEach((tb) => {
...
var _a5;
var BackgroundRenderer = (_a5 = class {
  constructor(deps) {
  ...
  */
  async renderBackgroundURLImage(container, backgroundImage2, index) {
    ...
    try {
      ...
      (_b2 = (_a8 = this.context).onError) == null ? void 0 : _b2.call(_a8, e2 instanceof Error ? e2 : new Error(String(e2)));
    }
    if (image2) {
  ...
  */
  ...
  lruSet(cache, key, value) {
    if (cache.size >= _a5.PATTERN_CACHE_MAX) {
      const oldestKey = cache.keys().next().value;
      cache.delete(oldestKey);
    }
    cache.set(key, value);
  }
}, _a5.PATTERN_CACHE_MAX = 50, _a5);
...
async function renderReplacedElements(ctx, context, options, iframeRendererFactory, container, curves, styles, renderReplacedElementFn) {
  ...
  if (container instanceof ImageElementContainer) try {
    ...
    (_a8 = context.onError) == null ? void 0 : _a8.call(context, e2 instanceof Error ? e2 : new Error(String(e2)));
  }
  ...
  if (container instanceof SVGElementContainer) try {
    ...
    (_b2 = context.onError) == null ? void 0 : _b2.call(context, e2 instanceof Error ? e2 : new Error(String(e2)));
  }
...
function renderFormElements(ctx, fontMetrics, textRenderer, pathFn, container, styles) {
  ...
  if (isTextInputElement(container) && container.value.length) {
    const [font, fontFamily2, fontSize2] = textRenderer.createFontStyle(styles);
    const { baseline } = fontMetrics.getMetrics(fontFamily2, fontSize2);
    ctx.font = font;
  ...
  */
  ...
  async renderStackContent(stack) {
    ...
    for (const child of stack.negativeZIndex) {
      if (signal == null ? void 0 : signal.aborted) throw new DOMException("The operation was aborted.", "AbortError");
...
var ForeignObjectRenderer = class {
  ...
  async render(element) {
    var _a8;
    if ((_a8 = this.options.signal) == null ? void 0 : _a8.aborted) throw new DOMException("The operation was aborted.", "AbortError");
...
var _a7;
var Context = (_a7 = class {
  constructor(options, windowBounds, config) {
    this.windowBounds = windowBounds;
    this.instanceName = `#${_a7.instanceCount++}`;
    this.config = config;
